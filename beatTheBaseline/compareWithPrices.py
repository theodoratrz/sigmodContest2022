...
with open("../datasets/X2.csv") as instancesFile:
    ...
    for row in instancesRows:
        instances[row[0]] = (row[1], row[2], row[3])
...
def mapPair(pair: List[str]):
    return {
        "title1": instances[pair[0]][0],
        "price1": instances[pair[0]][1],
        "brand1": instances[pair[0]][2],
        "title2": instances[pair[1]][0],
        "price2": instances[pair[1]][1],
        "brand2": instances[pair[1]][2]
    }

with open("missed.csv", mode="w") as missedCsv:
    #writer = csv.writer(missedCsv)
    writer = csv.DictWriter(missedCsv, fieldnames=["title1", "price1", "brand1", "title2", "price2", "brand2"])
    writer.writeheader()
    writer.writerows([mapPair(p) for p in missed])

with open("false.csv", mode="w") as falseCsv:
    writer = csv.DictWriter(falseCsv, fieldnames=["title1", "price1", "brand1", "title2", "price2", "brand2"])
    writer.writeheader()
    writer.writerows([mapPair(p) for p in falsePositives])
pass
