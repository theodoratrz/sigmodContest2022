...
#from x2_blocking import SOLVED_PAIR_SCORE
from utils import TARGET_DATASET

with open('../output_misc/output.csv') as outFile:
...
with open("../output_misc/missed.csv", mode="w") as missedCsv:
    writer = csv.writer(missedCsv)
    writer.writerows(missedTitles)

with open("../output_misc/false.csv", mode="w") as falseCsv:
    writer = csv.writer(falseCsv)
    writer.writerows(falseTitles)

with open("../output_misc/true.csv", mode="w") as trueCsv:
    writer = csv.writer(trueCsv)
    writer.writerows(trueTitles)

with open("../output_misc/common.csv", mode="w") as commonCsv:
